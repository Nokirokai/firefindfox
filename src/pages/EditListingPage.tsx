...
import { updateListing } from '../hooks/useListings'
...
export default function EditListingPage({
  ...
  // Pre-populate with existing images (marked as existing)
  ...
  if (!user || user.id !== listing.seller_id) {
    return (
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-20 text-center">
        <p className="text-sm text-[#737373]">You are not allowed to edit this listing.</p>
      </div>
    )
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const newImageFiles = images.filter((img) => !img.isExisting && img.file).map((img) => img.file as File)

    const result = await updateListing(listing.id, {
      title: title.trim(), description: description.trim(),
      price: priceNum, condition, category, location,
      existingImageUrls, newImageFiles, sellerId: user!.id,
    })
    setLoading(false)

    if (result.error) { setError(result.error) }
    else { setSuccess(true); setTimeout(() => setPage('listing'), 1500) }
  }
  ...
  if (success) {
    return (
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-20 text-center">
        ...
        <h2 className="text-lg font-bold text-[#0A0A0A] mb-2" style={{ fontFamily: MONO }}>Listing updated!</h2>
        <p className="text-sm text-[#737373]">Going back to your listing...</p>
      </div>
    )
  }
  ...
  const previewImage = images[0]?.preview ?? listing.image

  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      <button onClick={() => setPage('listing')} className="flex items-center gap-1.5 text-xs text-[#737373] hover:text-[#0A0A0A] transition-colors mb-6" style={{ fontFamily: MONO }}>
        <ChevronLeft size={13} /> Back to listing
      </button>
      <h1 className="text-2xl font-bold text-[#0A0A0A] mb-6" style={{ fontFamily: MONO }}>Edit Listing</h1>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" value={price} onChange={(e) => setPrice(e.target.value)}
                min="1"
                className="w-full border border-[#E5E5E5] bg-[#F5F5F5] px-4 py-2.5 text-sm text-[#0A0A0A] focus:outline-none focus:border-[#0A0A0A]"
